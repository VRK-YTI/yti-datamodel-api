...
class ModelMapperTest {
    ...
    void testMapToJenaModel(GraphType modelType) {
        var mockModel = ModelFactory.createDefaultModel();
        mockModel.createResource(Constants.DATA_MODEL_NAMESPACE + "newint" + Constants.RESOURCE_SEPARATOR)
        ...
        dto.setInternalNamespaces(Set.of(Constants.DATA_MODEL_NAMESPACE + "newint"));
        ...
        Resource modelResource = model.getResource(Constants.DATA_MODEL_NAMESPACE + "test" + Constants.RESOURCE_SEPARATOR);
        ...
        if(modelType.equals(GraphType.PROFILE)){
            ...
            assertTrue(requires.containsAll(
                    List.of("http://www.w3.org/2000/01/rdf-schema#",
                            "http://uri.suomi.fi/terminology/test",
                            Constants.DATA_MODEL_NAMESPACE + "newint"
                    )));
        }
    ...
    void testMapToUpdateJenaModel() {
        ...
        var mockModel = ModelFactory.createDefaultModel();
        mockModel.createResource(Constants.DATA_MODEL_NAMESPACE + "newint")
                .addProperty(RDF.type, OWL.Ontology)
                .addProperty(DCAP.preferredXMLNamespacePrefix, "test");
        when(coreRepository.fetch(anyString())).thenReturn(mockModel);
        ...
        assertEquals(1, modelResource.listProperties(OWL.imports).toList().size());
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "int/", modelResource.listProperties(OWL.imports).next().getObject().toString());
        ...
        assertEquals(1, modelResource.listProperties(OWL.imports).toList().size());
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "newint", modelResource.listProperties(OWL.imports).next().getObject().toString());
        assertEquals(mockUser.getId().toString(), MapperUtils.propertyToString(modelResource, SuomiMeta.modifier));
    ...
    void testMapToDatamodelDTO() {
        ...
        when(coreRepository.fetch(Constants.DATA_MODEL_NAMESPACE + "int/")).thenReturn(nsModel);
        var result = mapper.mapToDataModelDTO("test", m, null);

        assertEquals("test", result.getPrefix());
        assertEquals(GraphType.LIBRARY, result.getGraphType());
        ...
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "int/", internalDTO.getNamespace());
    ...
    void testMapToDatamodelDTOVersion() {
        ...
        assertEquals(Status.VALID, result.getStatus());
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "test/1.0.1/", result.getVersionIri());
    }
    ...
    void testMapReleaseProperties() {
        ...
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "test/1.0.1/", MapperUtils.propertyToString(resource, OWL2.versionIRI));
    ...
    void testMapPriorVersion() {
        ...
        var m = MapperTestUtils.getModelFromFile("/test_datamodel_library.ttl");
        mapper.mapPriorVersion(m, Constants.DATA_MODEL_NAMESPACE + "test", Constants.DATA_MODEL_NAMESPACE + "test/1.0.0/");

        var resource = m.getResource(Constants.DATA_MODEL_NAMESPACE + "test");
        assertEquals(Constants.DATA_MODEL_NAMESPACE + "test/1.0.0/", MapperUtils.propertyToString(resource, OWL.priorVersion));
    }
    ...
    void testMapToIndexModel() {
        ...
        var graphUri = Constants.DATA_MODEL_NAMESPACE + "test/";
        var result = mapper.mapToIndexModel(graphUri, m);

        assertEquals("test", result.getPrefix());
        assertEquals(graphUri, result.getId());
        assertEquals(GraphType.LIBRARY, result.getType());
        assertEquals(Status.VALID, result.getStatus());
    ...
    void mapModelVersionInfo() {
        ...
        var uri = Constants.DATA_MODEL_NAMESPACE + "test/1.0.0/";
    ...
    void mapChildOrganization() {
        ...
        var model = mapper.mapToJenaModel(dto, GraphType.LIBRARY, mockUser);
